...
export async function cargarSeguimientoCliente(clienteId) {
  ...
  const [fbRes, revisRes, molRes, notasRes] = await Promise.all([
    ...
    supabase.from('molestia_reportes').select('sesion_feedback_id, estado, episodio_id').eq('cliente_id', clienteId).not('sesion_feedback_id', 'is', null),
...
export async function cargarSeguimientoGlobal(clientes) {
  ...
  const [fbRes, revisRes, molRes] = await Promise.all([
    ...
    supabase.from('molestia_reportes').select('sesion_feedback_id, estado, episodio_id').in('cliente_id', clienteIds).not('sesion_feedback_id', 'is', null),
  ])
