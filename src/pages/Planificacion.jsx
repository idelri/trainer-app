...
export default function Planificacion({ clientePlanificacion }) {
 ...
 const [formSemana, setFormSemana] = useState({ objetivo: '', notas: '', comentario: '', carga: 'media', zona1_2_real: 0, zona3_4_real: 0, zona5_real: 0, km_objetivo: null, km_real: null })
 ...
 const [modalCopiar, setModalCopiar] = useState(false)
  ...
  async function cargarPlanificacion() {
    ...
    if (plan) {
      ...
      setPlanificacion(null); setBloques([]); setSemanas({}); setSubbloques({})
    }
  ...
  async function guardarComp() {
    ...
    setSaving(true)
    if (modalComp?.id) {
      await supabase.from('competiciones').update({ nombre: formComp.nombre, fecha: formComp.fecha, tipo: formComp.tipo || null, objetivo: formComp.objetivo || null, notas: formComp.notas || null }).eq('id', modalComp.id)
    } else {
      await supabase.from('competiciones').insert({ cliente_id: clienteSeleccionado, nombre: formComp.nombre, fecha: formComp.fecha, tipo: formComp.tipo || null, objetivo: formComp.objetivo || null, notas: formComp.notas || null })
    }
 ...
 async function eliminarComp(id) {
  ...
  function abrirNuevoBloque() {
    const fechaInicio = bloques.length > 0 ? format(addWeeks(parseISO(bloques[bloques.length - 1].fecha_inicio), bloques[bloques.length - 1].semanas), 'yyyy-MM-dd') : planificacion?.fecha_inicio || ''
    setFormBloque({ ...EMPTY_BLOQUE, fecha_inicio: fechaInicio }); setModalBloque({})
  }

  function abrirEditarBloque(b) {
    setFormBloque({ nombre: b.nombre, fase: b.fase, carga: b.carga, semanas: b.semanas, fecha_inicio: b.fecha_inicio, objetivo: b.objetivo || '', contenidos: b.contenidos || '', color: b.color || '#2d6a4f' })
    setModalBloque(b)
  }
  ...
  const totalSemanas = planificacion ? differenceInWeeks(parseISO(planificacion.fecha_fin), parseISO(planificacion.fecha_inicio)) + 1 : 0

  function ZonasSubbloque({ sub, semsDelSub }) {
    const totalMin = semsDelSub.reduce((s, x) => s + (x.zona1_2_real || 0) + (x.zona3_4_real || 0) + (x.zona5_real || 0), 0)
    const z1 = semsDelSub.reduce((s, x) => s + (x.zona1_2_real || 0), 0)
    const z3 = semsDelSub.reduce((s, x) => s + (x.zona3_4_real || 0), 0)
    const z5 = semsDelSub.reduce((s, x) => s + (x.zona5_real || 0), 0)
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {tieneObj && (
          <div>
            <div style={{ fontSize: 9, color: 'var(--text3)', fontFamily: 'var(--mono)', marginBottom: 2 }}>Objetivo</div>
        ...
        {tieneReal && (
          <div>
            <div style={{ fontSize: 9, color: 'var(--text3)', fontFamily: 'var(--mono)', marginBottom: 2 }}>Real — {totalMin} min</div>
            <div style={{ display: 'flex', height: 6, borderRadius: 3, overflow: 'hidden', marginBottom: 2 }}>
              {z1 > 0 && <div style={{ width: `${(z1 / totalMin) * 100}%`, background: '#10b981' }} />}
              {z3 > 0 && <div style={{ width: `${(z3 / totalMin) * 100}%`, background: '#f59e0b' }} />}
              {z5 > 0 && <div style={{ width: `${(z5 / totalMin) * 100}%`, background: '#ef4444' }} />}
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              {z1 > 0 && <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#10b981' }}>Z1-Z2 {Math.round((z1 / totalMin) * 100)}%</span>}
              {z3 > 0 && <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#f59e0b' }}>Z3-Z4 {Math.round((z3 / totalMin) * 100)}%</span>}
              {z5 > 0 && <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: '#ef4444' }}>Z5-Z5+ {Math.round((z5 / totalMin) * 100)}%</span>}
            </div>
          </div>
        )}
      </div>
  ...
  function FilaSemana({ b, num, semsBloque, subsBloque }) {
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 16px', cursor: 'pointer' }} onClick={() => abrirSemana(b.id, num)}>
          ...
          <div style={{ flex: 1 }}>
            {sem?.objetivo
              ...
              : <div style={{ fontSize: 13, color: 'var(--text3)', fontStyle: 'italic' }}>{subsBloque.find(s => num >= s.semana_inicio && num <= s.semana_fin)?.nombre || 'Clic para añadir objetivo'}</div>
            }
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      ...
                      <div>
                        <div style={{ fontSize: 10, color: 'var(--text3)', marginBottom: 3 }}>Real — {total} min</div>
                        <div style={{ display: 'flex', height: 8, borderRadius: 4, overflow: 'hidden' }}>
                          {sem.zona1_2_real > 0 && <div style={{ width: `${(sem.zona1_2_real / total) * 100}%`, background: '#10b981' }} />}
                          {sem.zona3_4_real > 0 && <div style={{ width: `${(sem.zona3_4_real / total) * 100}%`, background: '#f59e0b' }} />}
                          {sem.zona5_real > 0 && <div style={{ width: `${(sem.zona5_real / total) * 100}%`, background: '#ef4444' }} />}
                        </div>
                        <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
                          {sem.zona1_2_real > 0 && <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: '#10b981' }}>Z1-Z2 {Math.round((sem.zona1_2_real / total) * 100)}% ({sem.zona1_2_real}min)</span>}
                          {sem.zona3_4_real > 0 && <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: '#f59e0b' }}>Z3-Z4 {Math.round((sem.zona3_4_real / total) * 100)}% ({sem.zona3_4_real}min)</span>}
                          {sem.zona5_real > 0 && <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: '#ef4444' }}>Z5-Z5+ {Math.round((sem.zona5_real / total) * 100)}% ({sem.zona5_real}min)</span>}
                        </div>
                      </div>
                    </div>
                  )
  ...
  return (
    <div>
        ...
        <div className="flex gap-2" style={{ position: 'relative' }}>
          ...
          {planificacion && (
            <button className="btn btn-ghost" style={{ color: 'var(--danger)' }} onClick={async () => {
              ...
              setPlanificacion(null); setBloques([]); setSemanas({}); setSubbloques({}); setCompeticiones([]); cargarPlanificacion()
            ...
            {objetivoVisible.menuAnadir && (
              <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-md)', zIndex: 200, minWidth: 180, overflow: 'hidden' }}
                ...
                {planificacion && (<>
                  ...
                  <button className="btn btn-ghost" style={{ width: '100%', justifyContent: 'flex-start', borderRadius: 0, padding: '10px 16px' }}
                    onClick={() => { setObjetivoVisible(v => ({ ...v, menuAnadir: false })); if (bloques.length === 0) { alert('Primero crea un bloque'); return }; setFormSubbloque({ nombre: '', semana_inicio: 1, semana_fin: 1, objetivo: '', notas: '', zona1_2: 0, zona3_4: 0, zona5: 0, km_min: null, km_max: null, sesiones_min: null, sesiones_max: null, duracion_media_min: null, exigencia: '', enfoque: [], enfoque_prioridad: {} }); setModalSubbloque({ bloque_id: bloques[0].id }) }}>
      ...
      <div className="flex gap-3 items-center" style={{ marginBottom: 20 }}>
        <select className="form-select" style={{ maxWidth: 260 }} value={clienteSeleccionado || ''} onChange={e => { setClienteSeleccionado(e.target.value || null); setPlanificacion(null); setBloques([]) }}>
          <option value="">Selecciona un cliente...</option>
          {clientes.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
        </select>
        {planificaciones.length > 1 && (
          <select className="form-select" style={{ maxWidth: 260 }} value={planificacion?.id || ''} onChange={e => { const p = planificaciones.find(x => x.id === e.target.value); setPlanificacion(p || null); setBloques([]); setSemanas({}); setSubbloques({}) }}>
      ...
      {!loading && clienteSeleccionado && !planificacion && (
        <div className="empty">
          ...
          <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={() => { setFormPlan({ ...EMPTY_PLAN, cliente_id: clienteSeleccionado }); setModalPlan(true) }}>
                    ...
                    {bloques.map(b => {
                      ...
                      return (
                       <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                         <div style={{ fontSize: 13, fontWeight: 500, color: b.color || '#2d6a4f' }}>{b.nombre}</div>
                    ...
                    return (
                      <div style={{ position: 'relative', height: 20, marginBottom: 2 }}>
                        <div style={{ position: 'absolute', left: `${pct2}%`, transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', zIndex: 10 }}>
                          <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--accent)', fontWeight: 700, whiteSpace: 'nowrap', background: 'var(--accent-light)', padding: '1px 6px', borderRadius: 4, border: '1px solid var(--accent)' }}>▼ Estás aquí</div>
                        </div>
                      </div>
                    )
                  })()}
                  ...
                  <div style={{ display: 'flex', gap: 2, marginBottom: 3, position: 'relative', height: 28 }}>
                    {bloques.map(b => {
                      ...
                      return subsBloque.map(sub => {
                        ...
                        return (
                          <div key={sub.id} onClick={() => { setFormSubbloque({ nombre: sub.nombre, semana_inicio: sub.semana_inicio, semana_fin: sub.semana_fin, objetivo: sub.objetivo || '', notas: sub.notas || '', zona1_2: sub.zona1_2 || 0, zona3_4: sub.zona3_4 || 0, zona5: sub.zona5 || 0, km_min: sub.km_min || null, km_max: sub.km_max || null, sesiones_min: sub.sesiones_min || null, sesiones_max: sub.sesiones_max || null, duracion_media_min: sub.duracion_media_min || null, exigencia: sub.exigencia || '', enfoque: sub.enfoque || [], enfoque_prioridad: sub.enfoque_prioridad || {} }); setModalSubbloque({ ...sub, bloque_id: b.id }) }}
               ...
               {(competiciones.length > 0 || controles.length > 0) && (
                    <div style={{ display: 'flex', marginBottom: 3, position: 'relative', height: 36 }}>
                      {competiciones.map(comp => {
                        ...
                        return (
                          <div key={comp.id} onClick={() => { setFormComp({ nombre: comp.nombre, fecha: comp.fecha, tipo: comp.tipo || '', objetivo: comp.objetivo || '', notas: comp.notas || '' }); setModalComp({ ...comp }) }}
                  ...
                  <div style={{ display: 'flex', borderTop: '1px solid var(--border)', paddingTop: 2 }}>
                    {Array.from({ length: totalSemanas }, (_, i) => {
                      let sg = i + 1, bde = null, seb = null, ac = 0
                      for (const b of bloques) { if (sg > ac && sg <= ac + b.semanas) { bde = b; seb = sg - ac; break }; ac += b.semanas }
                      return (
                        <div key={i} onClick={() => { if (bde) { setVista('micro'); setBloqueAbierto(bde.id) } }}
                          style={{ flex: 1, textAlign: 'center', fontSize: 8, color: bde ? 'var(--accent)' : 'var(--text3)', fontFamily: 'var(--mono)', borderLeft: i % 4 === 0 ? '1px solid var(--border)' : 'none', cursor: bde ? 'pointer' : 'default', padding: '2px 0', borderRadius: 2 }}
                          onMouseEnter={e => { if (bde) e.currentTarget.style.background = 'var(--accent-light)' }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'transparent' }}
                          title={bde ? `${bde.nombre} · Semana ${seb}` : ''}>{`S${i + 1}`}</div>
                      )
                    })}
                  </div>
                  ...
                  <div style={{ display: 'flex', marginTop: 2 }}>
                    {Array.from({ length: totalSemanas }, (_, i) => {
                      ...
                      const esPrimero = i === 0 || format(fecha, 'MM') !== format(addWeeks(parseISO(planificacion.fecha_inicio), i - 1), 'MM')
                      return (
                        <div key={i} style={{ flex: 1, position: 'relative', height: 16 }}>
                          {esPrimero && <div style={{ position: 'absolute', left: 0, top: 0, fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--accent)', whiteSpace: 'nowrap', textTransform: 'capitalize', borderLeft: '1px solid var(--accent)', paddingLeft: 3, fontWeight: 600 }}>{format(fecha, 'MMM yyyy', { locale: es })}</div>}
                        </div>
                      )
                    })}
                  </div>
                ...
                return (
                  <div key={b.id} className="card" style={{ padding: 0, overflow: 'hidden', borderLeft: `4px solid ${b.color || '#2d6a4f'}` }}>
                    <div style={{ padding: '12px 16px', background: 'var(--bg)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }} onClick={() => setBloqueAbierto(abierto ? null : b.id)}>
                        ...
                        {b.objetivo && <button className="btn btn-ghost btn-sm" onClick={e => { e.stopPropagation(); setObjetivoVisible(v => ({ ...v, [b.id]: !v[b.id] })) }} style={{ color: objetivoVisible[b.id] ? b.color || 'var(--accent)' : 'var(--text3)', padding: '2px 6px' }}><Layers size={13} /></button>}
                        <button className="btn btn-ghost btn-sm ml-auto" onClick={e => { e.stopPropagation(); setFormSubbloque({ nombre: '', semana_inicio: 1, semana_fin: 1, objetivo: '', notas: '', zona1_2: 0, zona3_4: 0, zona5: 0, km_min: null, km_max: null, sesiones_min: null, sesiones_max: null, duracion_media_min: null, exigencia: '', enfoque: [], enfoque_prioridad: {} }); setModalSubbloque({ bloque_id: b.id }) }}>
                    ...
                    {abierto && (
                      <div style={{ borderTop: '1px solid var(--border)' }}>
                        {subsBloque.length === 0 ? (
                          <div style={{ padding: '16px', color: 'var(--text3)', fontSize: 13, fontStyle: 'italic' }}>Sin sub bloques — añade el primero.</div>
                        ) : subsBloque.map(sub => {
                            ...
                            <div key={sub.id} style={{ borderBottom: '1px solid var(--border)' }}>
                              <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                <div className="flex gap-1 ml-auto">
                                  ...
                                  <button className="btn btn-ghost btn-sm" onClick={() => { setFormSubbloque({ nombre: sub.nombre, semana_inicio: sub.semana_inicio, semana_fin: sub.semana_fin, objetivo: sub.objetivo || '', notas: sub.notas || '', zona1_2: sub.zona1_2 || 0, zona3_4: sub.zona3_4 || 0, zona5: sub.zona5 || 0, km_min: sub.km_min || null, km_max: sub.km_max || null, sesiones_min: sub.sesiones_min || null, sesiones_max: sub.sesiones_max || null, duracion_media_min: sub.duracion_media_min || null, exigencia: sub.exigencia || '', enfoque: sub.enfoque || [] }); setModalSubbloque({ ...sub, bloque_id: b.id }) }}>Editar</button>
                                          ...
                                          {(sub.km_min || sub.km_max) && (
                                            <div>
                                              ...
                                              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 2 }}>
                                                ...
                                                {sub.km_min && sub.km_max && <span style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--text3)', margin: '0 4px' }}>–</span>}
                    ...
                    {abierto && (
                      <div style={{ borderTop: '1px solid var(--border)' }}>
                        {subsBloque.length > 0 ? subsBloque.map(sub => {
                          const semsDelSubNums = Array.from({ length: sub.semana_fin - sub.semana_inicio + 1 }, (_, i) => sub.semana_inicio + i)
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group"><label className="form-label">Cliente *</label>
              <select className="form-select" value={formPlan.cliente_id} onChange={e => setFormPlan(f => ({ ...f, cliente_id: e.target.value }))}>
                <option value="">Selecciona...</option>{clientes.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}
              </select>
            </div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 4 }}>
                {COLORES.map(c => <div key={c} onClick={() => setFormBloque(f => ({ ...f, color: c }))} style={{ width: 28, height: 28, borderRadius: '50%', background: c, cursor: 'pointer', border: formBloque.color === c ? '3px solid var(--text)' : '3px solid transparent', transition: 'border 0.15s' }} />)}
              </div>
            </div>
            <div className="form-row">
            ...
            <div className="form-group"><label className="form-label">Objetivo del bloque</label><textarea className="form-textarea" value={formBloque.objetivo} onChange={e => setFormBloque(f => ({ ...f, objetivo: e.target.value }))} placeholder={"Ej: Desarrollar base aeróbica\nMejorar técnica de carrera"} /></div>
            <div className="modal-footer">
     ...
     {modalSemanaTipo && (
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
           ...
           {['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'].map(dia => {
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={async () => { setSaving(true); await supabase.from('clientes').update({ semana_tipo: formSemanaTipo, disponibilidad: formDisponibilidad }).eq('id', clienteSeleccionado); setSaving(false); setModalSemanaTipo(false); cargarClienteData(clienteSeleccionado) }} disabled={saving}>{saving ? 'Guardando...' : 'Guardar'}</button>
            </div>
          </div>
        </div>
      )}
      {modalConsideraciones && (
        <div className="modal-backdrop" onClick={() => setModalConsideraciones(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Consideraciones</span><button className="btn btn-ghost btn-sm" onClick={() => setModalConsideraciones(false)}><X size={14} /></button></div>
            <div className="form-group"><label className="form-label">Una consideración por línea</label><textarea className="form-textarea" style={{ minHeight: 160 }} value={formConsideraciones} onChange={e => setFormConsideraciones(e.target.value)} placeholder={"Ej: Molestia en rodilla derecha\nViaje previsto en octubre"} /></div>
            <div className="modal-footer">
              <button className="btn btn-ghost" onClick={() => setModalConsideraciones(false)}>Cancelar</button>
              <button className="btn btn-primary" onClick={async () => { setSaving(true); await supabase.from('clientes').update({ consideraciones: formConsideraciones }).eq('id', clienteSeleccionado); setSaving(false); setModalConsideraciones(false); cargarClienteData(clienteSeleccionado) }} disabled={saving}>{saving ? 'Guardando...' : 'Guardar'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalSemana && (
        <div className="modal-backdrop" onClick={() => setModalSemana(null)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-header"><span className="modal-title">Semana {modalSemana.numero}</span><button className="btn btn-ghost btn-sm" onClick={() => setModalSemana(null)}><X size={14} /></button></div>
            <div className="form-group"><label className="form-label">Carga</label>
            ...
            <div className="form-group"><label className="form-label">Objetivo de la semana</label><input className="form-input" value={formSemana.objetivo} onChange={e => setFormSemana(f => ({ ...f, objetivo: e.target.value }))} placeholder="Ej: Aumentar volumen de carrera" autoFocus /></div>
           {clienteData?.perfil_planificacion !== 'fuerza_salud' && (
              <div className="form-row">
                <div className="form-group"><label className="form-label">Km objetivo</label><input className="form-input" type="number" min="0" value={formSemana.km_objetivo || ''} onChange={e => setFormSemana(f => ({ ...f, km_objetivo: e.target.value ? parseInt(e.target.value) : null }))} placeholder="Ej: 22" /></div>
                <div className="form-group"><label className="form-label">Km real</label><input className="form-input" type="number" min="0" value={formSemana.km_real || ''} onChange={e => setFormSemana(f => ({ ...f, km_real: e.target.value ? parseInt(e.target.value) : null }))} placeholder="Ej: 24" /></div>
              </div>
            )}
              ...
              {[{ key: 'zona1_2_real', label: 'Z1-Z2', sublabel: 'Base / Recuperación', color: '#10b981' }, { key: 'zona3_4_real', label: 'Z3-Z4', sublabel: 'Umbral / Calidad', color: '#f59e0b' }, { key: 'zona5_real', label: 'Z5-Z5+', sublabel: 'Alta intensidad', color: '#ef4444' }].map(zona => {
                ...
                return (
                  <div key={zona.key} style={{ marginBottom: 10 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      <div><span style={{ fontSize: 12, fontWeight: 600, color: zona.color }}>{zona.label}</span><span style={{ fontSize: 11, color: 'var(--text3)', marginLeft: 6 }}>{zona.sublabel}</span></div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        {total > 0 && <span style={{ fontSize: 11, fontFamily: 'var(--mono)', color: zona.color }}>{pct}%</span>}
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <input type="number" min="0" max="600" value={formSemana[zona.key] || 0} onChange={e => setFormSemana(f => ({ ...f, [zona.key]: parseInt(e.target.value) || 0 }))} style={{ width: 64, padding: '4px 8px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', fontFamily: 'var(--mono)', fontSize: 13, textAlign: 'right' }} />
                          <span style={{ fontSize: 11, color: 'var(--text3)' }}>min</span>
                        </div>
                      </div>
                    </div>
                    {total > 0 && <div style={{ height: 6, background: 'var(--bg2)', borderRadius: 3, overflow: 'hidden' }}><div style={{ height: '100%', width: `${pct}%`, background: zona.color, borderRadius: 3 }} /></div>}
                  </div>
                )
              })}
