  ...
  return (
    <div ref={el => registerRef && registerRef(sub.id, el)} style={{ scrollMarginTop: 80, borderTop: `1px solid ${T.border}` }}>
      <div onClick={onToggle} style={{ padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', background: isOpen ? (b.color || T.green) + '0c' : 'transparent' }}>
       ...
       <span style={{ fontFamily: T.mono, fontSize: 10, color: col, fontWeight: 600, flexShrink: 0 }}>SB{subIdx + 1}</span>
        ...
        <div style={{ ...card, borderLeft: `4px solid ${colA}`, padding: '15px 16px', overflow: 'hidden' }}>
          ...
          {enCurso ? (
            <>
             ...
             <button onClick={() => {
                  const rel = semanaActual - meta[bActualIdx].startGlobal + 1
                  jump({ block: bActual.id, sub: subActual?.id, week: `w-${bActual.id}-${rel}` })
                }} style={{ marginTop: 11, display: 'inline-flex', alignItems: 'center', gap: 6, background: colA, border: 'none', color: '#fff', fontFamily: T.font, fontSize: 12.5, fontWeight: 500, padding: '7px 13px', borderRadius: 8, cursor: 'pointer' }}>
                  📍 Semana actual
